...
class Scanner():
    """
    Class used to parse the folder and compute the pre-fire and post-fire dates of events. The folder must contain subfolders associated to satellite acquisitions and each subfolder contains images in .tiff with the following naming convention: product-name_yyyy_mm_dd[_cloud_coverage].tiff. The _cloud_coverage option is available only in sentinel2 products.
    ...
    def scan_folder(self, folder: str) -> dict:
        """
        Scan the content of specified subfolder to identify all the products of interest files (element of self.products). For each product, the files are analyzed to find the pre-fire and post-fire acquisition dates.

        Args:
            folder (str): fullpath of the folder to be analyzed
        Returns:
            dict(dict(str)): returns a dict containing for each product a dict with two keys: 'pre' and 'post'. If the value associated to 'pre' or 'post' is empty, the file was not found. To decide whether an acquisition is pre-fire or post-fire, the min and max dates are computed. If only one single file was found for a product, if df_path was specified, the activation_date value is used. Otherwise, the file is associated to post-fire.
        """
        result = {}
    ...
    def scan_master_folder(self): 
        ...
        for dirname in os.listdir(self.folder):
            ...
            if self.ignore_list is not None and dirname in list(self.ignore_list):
                continue

            if self.valid_list is not None and dirname not in list(self.valid_list):
                continue
    ...
    def get(self, folder: str, product: str, mode: str) -> np.ndarray:
        """
        ...
        if product != 'cloud_coverage':
            filename = tmp_product + '_' + date + '.png'
        else:
            filename = tmp_product + '_' + date + '_cloud_coverage.png'
